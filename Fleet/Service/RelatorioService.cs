...
namespace Fleet.Service
{
    public class RelatorioService(IRelatorioRepository relatorioRepository, IConfiguration configuration, IUsuarioRepository usuarioRepository, ILoggedUser loggedUser, IBucketService bucketService) : IRelatorioService
    {
        ...
        {
            string fileName = Guid.NewGuid() + "_relatorio-visita";
            string path = AppDomain.CurrentDomain.BaseDirectory + "\\images\\report";
            ...
            foreach(var relatorioVisita in relatorioVisitas) {
                ...
                foreach (var imagem in relatorioVisita.Imagens) {
                    if (!imagem.FotoAssinatura)
                        imagesHtml += $@"<img src='https://juriseg.ddns.net:3308/visit/{imagem}'>";
                    else
                        imagemAssinatura = $"https://juriseg.ddns.net:3308/signature/{imagem}";
                }
                ...
                string maps = $"https://www.google.com/maps?q=${relatorioVisita.GPS}=13&output=embed";
            ...
            var htmlContent = relatorio.ToString(); // Certifique-se de que isso é uma string
            ...
            using (var stream = new MemoryStream())
            {
                using (var writer = new StreamWriter(stream, Encoding.UTF8, leaveOpen: true))
                {
                    ...
                    await writer.FlushAsync(); // Escreve no stream
                }

                stream.Position = 0; // Reseta a posição do stream para o início

                // Chama o método de upload com o stream
